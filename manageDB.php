	...
	//if($link = mysqli_connect('localhost', 'root', '', 'assignment')){
		...
		if($res = mysqli_query($link, $query)){
			...
			if($row[0] < $N){
				...
				$query = "SELECT MAX(ID) FROM machines";	
				for($i = $N; $i < $row[0]; $i++){
					$res = mysqli_query($link, $query);
					if(!$res || mysqli_num_rows($res) != 1)
						die("<h1>Can't find maximun machine ID</h1>");
					...
					if(!mysqli_query($link, $deleteQuery))
						die("<h1>Elimination from reservations table failed</h1>");
					if(!mysqli_query($link, $deleteQuery1))
						die("<h1>Elimination from machines table failed</h1>");
					mysqli_free_result($res);
				}
